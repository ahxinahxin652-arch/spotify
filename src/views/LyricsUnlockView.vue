...
const outputPath = ref(localStorageStore.lyricsUnlockOutput)
let logId = 0
...
async function handleFileInput(e) {
  ...
  const result = await window.electronAPI.scanLyricsDecryptFiles(filePaths)
...
function handleDrop(e) {
  ...
  window.electronAPI.scanLyricsDecryptFiles(filePaths).then(result => {
    if (result.success && result.data) {
      const newFiles = result.data.files.filter(nf =>
          !files.value.some(ef => ef.path === nf.path)
      )
...
async function selectOutputPath() {
  ...
  if (path) {
    ...
    localStorageStore.setLyricsUnlockOutput(path)
...
async function startDecrypt() {
  ...
  const result = await window.electronAPI.startLyricsDecrypt({
    files: pureFiles,
    ...
    <div class="panel panel-files">
      ...
      <div class="panel-header">
        ...
        <div class="header-actions">
          ...
          <input type="file" ref="fileInputRef" multiple accept=".krc" style="display:none" @change="handleFileInput" />
        </div>
      </div>
      ...
      >
        <div class="drop-content">
          ...
          <p>点击或拖拽加密歌词文件到此处</p>
          <span class="drop-hint">支持: krc</span>
        </div>
      </div>
